...
class Auxiliary_sectMCAnalyses(QtWidgets.QMainWindow):
    ...
    def fiberMeshPlot(self,coreFiberInfo, innerFiberInfo, outFiberInfo,barFiberInfo, corePoints, innerPoints,
        ...
        pt1 = self.fiberMesh_ax.transData.transform((1, 0))  # X方向一个单位
        dx_pixels = pt1[0] - pt0[0]
        inches_per_data = dx_pixels / self.fiberMesh_figure.dpi  # 转换为英寸
        points_per_data = inches_per_data * 72  # 转换为点数
...
class PolygonOffset():
    ...
    def compute_polygon_area(self,vertices):
        """
        计算多边形有向面积，用于判断顶点顺序（正值：逆时针，负值：顺时针）
        """
        area = 0.0
        n = len(vertices)
        for i in range(n):
    ...
    def returnOffsetPolygonCoordsList(self):
        ...
        # 通过面积判断多边形的顶点顺序（面积 > 0 为逆时针，内侧在边的左侧）
        area = self.compute_polygon_area(vertices)
        factor = 1 if area > 0 else -1

        # 对每条边计算平移后的直线（使用直线的参数方程表示）
        # 对边 i: 由 vertices[i] 到 vertices[(i+1)%n]
        offset_lines = []
        ...
        # 对每个原始顶点，计算相邻两条 offset 直线的交点
        new_vertices = []
        for i in range(n):
            # 上一条边对应的直线（与当前顶点相邻的上一条边）和当前边的直线
            P1, d1 = offset_lines[i - 1]  # Python 中列表索引 i-1 表示上一个
            P2, d2 = offset_lines[i]

            inter_pt = self.line_intersection(P1, d1, P2, d2)
            if inter_pt is None:
                # 若两条直线平行，则在此情况下手动选择顶点进行调整（避免抛弃平行直线的平移计算）
                inter_pt = (P1 + P2) / 2
